...
export function buildDashboardCards(stats) {
  return [
    ...
    },
    {
      ...
      note: `${stats.totalRevenue.toFixed(2)} DT revenus aujourd’hui`,
      iconKey: "clipboard",
    },
    {
      title: "Utilisateurs",
...
export function buildOrdersRevenueChartData(orders = []) {
  const today = new Date();
  const days = [];
  ...
  const grouped = new Map(days.map((day) => [day.key, day]));
  ...
  orders.forEach((order) => {
    ...
    current.commandes += 1;
    current.revenus += Number(order.total_price || 0);
  });
